...
window.__lock={
  ...
  setFlat(on){                        // 2D: the same watch, head-on, no depth, no orbit
    flat=!!on;
    // A very narrow field of view from far away is, to the eye, an orthographic view: the case
    // shows no perspective, the bow and the dial sit in one plane. The fit keeps the size.
    camera.fov = flat ? 4 : 26; camera.far = flat ? 600 : 100; camera.updateProjectionMatrix();
    fitCamera(camera.aspect);
    caster.castShadow = !flat;        // no cast shadow on a flat wall
  },
...
function applyCamera(){
  ...
  const q = flat ? qFlat : qSmooth;   // 2D: the orbit is not applied, the bow still rocks
