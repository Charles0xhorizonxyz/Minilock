...
so the release velocity is high). Since v0.0.63 nothing but a dial gesture set to Unlock
dismisses (a swipe up used to); one of the two gestures is always Unlock, kept so by
`Gestures.choose` in the app and `Gestures.actionFor` on the lock screen. `BootReceiver` also restarts the service on `MY_PACKAGE_REPLACED`, so the watch is
...
- The phone sleeps; a black screenshot means that. Send `KEYCODE_WAKEUP`, and expect the
  stand-in lock screen on wake — a swipe no longer dismisses it (v0.0.63); use the Unlock
  gesture through the page, or `am force-stop` the app for the moment.
...
| `InfoBridge.java` | The phone's next event, alerts and alarm into the page (`__lock.setInfo`), every minute and on change. |
| `AlertsListener.java` | Notification listener: count and app names of clearable notifications, for the guichet and the alerts row. |
| `LockScreenActivity.java` | Stand-in lock screen. `showWhenLocked`; only a dial gesture set to Unlock dismisses, no swipe. |
