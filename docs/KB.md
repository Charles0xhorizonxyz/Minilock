...
Written 2026-09-15 at v0.0.63, current at v0.0.67. Keep it current: when something here changes, change it here too.
...
- v0.0.67 on the phone: the text under the watch is real (calendar, notifications, alarm) and inked against the chosen background; the background is separate from the Design; "Recenter only" resets size and position and nothing else; the weather counter is still the prototype's. The 2D "held by the ring" watch goes the whole way round; only a dial
...
- Every build is archived in `artifacts/` and logged in `artifacts/BUILDS.md`; every request and
  its honest status is a row in `docs/CORRECTIONS.md` (86 rows so far).
