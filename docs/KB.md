...
Written 2026-09-15 at v0.0.63. Keep it current: when something here changes, change it here too.
...
- v0.0.63 on the phone. The 2D "held by the ring" watch goes the whole way round; only a dial
...
- Every build is archived in `artifacts/` and logged in `artifacts/BUILDS.md`; every request and
  its honest status is a row in `docs/CORRECTIONS.md` (83 rows so far).
