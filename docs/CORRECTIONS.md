...
Every correction and request since the first version, with an honest status. Updated 2026-09-14, app at **v0.0.44**.
...
| 67 | Two turns left to right open the camera; make both gestures the user's choice, with a dropdown of everything available | **Done and verified** — v0.0.44. Two rows in the app under "Stand-in lock screen": "Flick right to left" (default Unlock) and "Flick left to right, twice" (default Open the camera), each a dropdown of Nothing, Unlock, Open the camera, Torch on or off, Open Minilock, Open the alarms. The page names the gesture ("left1", "right2": two flicks within a second and a half) and the lock screen performs the chosen action. Verified on the phone: the double flick opened the GrapheneOS camera and a single rightward flick only turned the watch over. Torch, Open Minilock and Open the alarms are wired the same way but were not exercised. |
