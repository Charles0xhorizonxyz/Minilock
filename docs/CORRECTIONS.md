...
Every correction and request since the first version, with an honest status. Updated 2026-09-14, app at **v0.0.52**.
...
| 75 | A toggle for 3D or 2D, and one for the gyroscope | **Done and verified** — v0.0.52. "3D watch" off shows the flat Canvas dial (the screensaver's design) on the app screen, the preview and the lock screen: no WebGL, no gyroscope, no bridges, no flick gestures; swipe up still unlocks and the stay/fade clock fades it natively. "Gyroscope" off never starts the sensor on any surface; switching it off live puts the camera back head-on. Verified on the phone: with the flat dial there is no WebView at all; with the gyroscope off, neither the app nor the staged lock screen registered the sensor. |
