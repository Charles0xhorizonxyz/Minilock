...
Every correction and request since the first version, with an honest status. Updated 2026-09-14, app at **v0.0.55**.
...
| 75 | A toggle for 3D or 2D, and one for the gyroscope | **Done and verified** — v0.0.52, corrected twice. v0.0.52 read "2D" as the old flat Canvas dial; v0.0.53 as a flat head-on camera with no gyroscope; you wanted the same watch with the gyroscope working, only as if its back were on a fixed surface. Since v0.0.54 "3D watch" off means exactly that: same perspective, same shadow, the gyroscope's orbit clamped to 65 degrees each way so you can look at it from an angle but never from behind, and the studio wall fixed in the world (scaled up so an oblique view never reaches its edge) with the watch lying on it. Live on every surface. "Gyroscope" off never starts the sensor on any surface. Verified on the phone: at a 120-degree orientation 3D shows the caseback from behind while 2D stops at the wall's limit with the wall seen at an angle; with the gyroscope off neither the app nor the staged lock screen registers the sensor. |
