...
Every correction and request since the first version, with an honest status. Updated 2026-09-14, app at **v0.0.53**.
...
| 75 | A toggle for 3D or 2D, and one for the gyroscope | **Done and verified** — v0.0.52, corrected in v0.0.53. v0.0.52 read "2D" as the old flat Canvas dial; you wanted the same watch. Since v0.0.53 "3D watch" off shows the pocket watch flat and head-on: a 4-degree field of view from far away, refitted to the same size, no cast shadow, and the gyroscope's orbit not applied (the bow still rocks with motion). Live on every surface. "Gyroscope" off never starts the sensor on any surface; switching it off live puts the camera back head-on. Verified on the phone: in 2D a pretend 45-degree turn changes nothing and the case shows no perspective or shadow; with the gyroscope off neither the app nor the staged lock screen registers the sensor. |
