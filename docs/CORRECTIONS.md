...
Every correction and request since the first version, with an honest status. Updated 2026-09-15, app at **v0.0.62**.
...
I solved the wrong problem for several versions. You meant **the app screen** — what opens when you tap Miniwatch — and I kept making the *watch's 3D camera* zoom instead.
...
| 25 | "Miniwatch keeps stopping" | Done — `getInsetsController()` before `setContentView` returns null |
...
| 52 | The watch colour and the background reset on unlock | **Done and verified for the colour; could not reproduce for the background** — v0.0.30. The colour (and the movement toggles and the counter) are chosen on the plate on the caseback, and nothing had ever saved that plate: it lived only in the page, so every new lock screen page started over. The page now hands the plate to the app through a `miniwatch` JavaScript interface on every change, and the app restores it on load. Verified: rose gold chosen in the app, sleep, wake, and the fresh lock screen page came back rose. The background was already restored from prefs on every load, and it also survived the same sleep-and-wake at a light grey. If it still resets for you, tell me the exact steps. |
...
| 67 | Two turns left to right open the camera; make both gestures the user's choice, with a dropdown of everything available | **Done and verified** — v0.0.44, corrected in v0.0.45. Two rows in the app under "Stand-in lock screen": "Flick right to left" (default Unlock) and "Spin left to right, two turns" (default Open the camera), each a dropdown of Nothing, Unlock, Open the camera, Torch on or off, Open Miniwatch, Open the alarms. v0.0.44 read the second gesture as two separate flicks; v0.0.45 makes it one action: a hard flick that carries the watch through two full turns (it fires once the spin crosses the barrier before the second full turn, and disarms if the watch comes to rest short of it). Verified on the phone: a gentle flick only turns the watch over, a hard one opened the GrapheneOS camera. Torch, Open Miniwatch and Open the alarms are wired the same way but were not exercised. How hard a flick has to be is set by the watch physics; say if two turns are hard to reach with a real finger. |
...
| 80 | Held by the ring, but moving in 3D | **Done, pretend turns verified; the feel is yours to judge** — v0.0.59, tuned in v0.0.60. The "3D watch" switch is now a "Watch motion" dropdown: Floating (as before), Held by the ring, 3D (new), Held by the ring, 2D (the pendulum). Held in 3D keeps the camera still, as 2D does, and hangs the watch from the ring: it swings sideways and in depth toward true gravity (lying the phone flat leans it into the screen, up to 40°, so the dial stays readable), and when the phone turns about the vertical the watch keeps its heading for a moment, showing its side, then twists back to face you over a few seconds (v0.0.59 came back within a second, too brisk to see much). Verified on the phone with pretend turns: a 60° turn kicks the twist to exactly −60° and it unwinds as designed; standing the phone up lowers the lean; a roll swings it sideways with the same sign as the 2D pendulum. Screenshots show the lean, the ring nearest, and the shadow following. The feel in the hand, the lean limit and the unwind speed need your hands. |
| 81 | In 2D the watch does not make a full 360 degree circle | **Done and verified with pretend turns; the feel is yours to judge** — v0.0.61. The pendulum was stopped at 1.3 rad (about 75 degrees) each side of upright, and its spring measured the plain difference to the gravity angle, which flips at 180 degrees. Now, held in 2D, the spring pulls the short way round, the angle wraps instead of stopping, and the whole circle is reachable: upside down the watch hangs upside down. Because a body hung from a ring fixed to the screen would leave the frame as soon as the phone lay on its side (that was #75's note), the body now keeps its place and the ring goes round it; it is the same swing seen from the body, with the same spring and the same push from motion, so tilting the phone no longer carries the body sideways off the screen. Lying flat, the old code bent the angle toward upright, which with a full circle would have turned an upside-down phone's watch through 90 degrees on its way down to the table; now within about 7 degrees of flat the spring lets go and the watch keeps its angle, as a real one does on a table, and the pull grows to full by 45 degrees. 3D by the ring and Floating are unchanged. Verified on the phone, installed at 16:00: with the phone flat on the table the spring reads slack and the watch holds still; stood up by a pretend turn it settles at 2 degrees; rolled 180 it reads -164 on its way to -178 and the screenshot shows the watch hanging upside down, ring at the bottom, body centred; rolled 90 it reads -96 (on its side); rolled -135 it reads 150 toward 137, having gone the short way round through 180 rather than back through zero. Cleared while flat it kept 147 degrees until stood up again, which is the table behaviour by design. One thing the screenshots show: at 90 degrees the ring and pendant poke past the side of the frame, because the fit is sized for the body (2.62 wide) and not for a ring that can point sideways (reach 1.79 from the centre). Fitting the ring at every angle would make the watch about 30 percent smaller in 2D; pinch out does the same by hand. Say which you prefer. The feel in the hand is yours. `__lock.hang()` reads the pendulum's angle in degrees for tests from adb. |
| 82 | Rename it Miniwatch | **Done and installed** — v0.0.62. The app is Miniwatch everywhere: launcher label, package and applicationId `com.miniscreen.miniwatch`, the screensaver service, the icon, the preferences file, the page's bridge, the tools, the docs and the memory notes. The dial still says MINISCREEN. Because the package changed, Android sees a new app: it was installed fresh, your fifteen settings were copied across from the old one (design, background, plate, gestures, motion, stay and fade, lock on), "display over other apps" was re-granted, the screensaver choice was re-pointed at the new service, and the old Minilock was uninstalled; the lock service came back on its own. The live wallpaper was not in use, so nothing to re-pick. Verified: package present at 0.0.62, settings read back, no crashes, service running. Not renamed: the GitHub repository (still `Minilock`, the docs keep that URL until you rename it on GitHub, which is a click under the repository's settings) and the old APK files in `artifacts/`. |
